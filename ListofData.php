...
<span style="font-size:22px;">List - Data File</span>
</nav>


<div class="container-fluid">
    <a href="UploadDataFile.php" style="margin-bottom:20px; color:#FFF;" class="btn btn-success a-btn-slide-text">
        <span class="fa fa-plus"></span><strong>&nbsp;&nbsp;Add Data</strong>
    </a>
...
if(isset($_POST['update'])){
  ...
  if($sort == 1){
    if($company != 0){
      if($branch == 0){
        $sql = "SELECT * FROM data_list WHERE company_id='$company'";
      }
      else{
        $sql = "SELECT * FROM data_list WHERE company_id='$company' AND branch_id='$branch'";
      }
    }
  }
  if($sort == 2){
    if($company != 0){
      if($branch == 0){
        $sql = "SELECT * FROM data_list WHERE company_id='$company' AND date BETWEEN '$startdate' AND '$enddate'";
      }
      else{
        $sql = "SELECT * FROM data_list WHERE company_id='$company' AND branch_id='$branch' AND date BETWEEN '$startdate' AND '$enddate'";
      }
    }
  }
  if($sort == 3){
    if($company != 0){
      if($branch == 0){
        ...
        $sql = "SELECT * FROM data_list WHERE company_id='$company' AND user_id='$newid'";
      }
      else{
        $sel = "select * from users_table where email='$email'";
        $runu = mysqli_query($con,$sel);
        ...
        $sql = "SELECT * FROM data_list WHERE company_id='$company' AND branch_id='$branch' AND user_id='$newid'";
      }
    }
  }
